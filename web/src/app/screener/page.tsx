...
function SectorBadge({ sector }: { sector: string }) {
  const colors: Record<string, string> = {
    "Information Technology": "text-[var(--color-text-secondary)] bg-[var(--color-surface-raised)] border border-[var(--border)]",
    "Health Care": "text-[var(--color-text-secondary)] bg-[var(--color-surface-raised)] border border-[var(--border)]",
    "Financials": "text-[var(--color-text-secondary)] bg-[var(--color-surface-raised)] border border-[var(--border)]",
    "Consumer Discretionary": "text-[var(--color-text-secondary)] bg-[var(--color-surface-raised)] border border-[var(--border)]",
    "Consumer Staples": "text-[var(--color-text-secondary)] bg-[var(--color-surface-raised)] border border-[var(--border)]",
    "Industrials": "text-[var(--color-text-secondary)] bg-[var(--color-surface-raised)] border border-[var(--border)]",
    "Energy": "text-[var(--color-text-secondary)] bg-[var(--color-surface-raised)] border border-[var(--border)]",
    "Materials": "text-[var(--color-text-secondary)] bg-[var(--color-surface-raised)] border border-[var(--border)]",
    "Real Estate": "text-[var(--color-text-secondary)] bg-[var(--color-surface-raised)] border border-[var(--border)]",
    "Utilities": "text-[var(--color-text-secondary)] bg-[var(--color-surface-raised)] border border-[var(--border)]",
    "Communication Services": "text-[var(--color-text-secondary)] bg-[var(--color-surface-raised)] border border-[var(--border)]",
  };
  const cls = colors[sector] ?? "text-[var(--color-text-secondary)] bg-[var(--color-surface-raised)] border border-[var(--border)]";
  return (
    ...
    <div className="flex flex-wrap gap-1.5">
      {items.slice(0, 3).map((item) => (
        <span
          ...
          className="inline-flex items-center gap-1.5 rounded-full border border-[var(--border)] bg-[var(--color-surface)] px-2.5 py-1 text-[10px] text-[var(--color-text-primary)] shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]"
        >
  ...
  return (
    <div className="mt-4 pt-4 border-t border-[var(--border)]/70 flex flex-col gap-3">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <p className="text-[11px] uppercase tracking-[0.24em] text-[var(--color-text-secondary)] font-semibold">
          Feature guide
        </p>
        <p className="text-[12px] md:text-[13px] text-[var(--color-text-secondary)] max-w-2xl leading-relaxed">
      ...
      <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
        {items.map((item) => (
          ...
          >
            ...
            <div className="mt-1 text-[var(--color-text-primary)]/90 leading-relaxed">{item.desc}</div>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="max-w-[1800px] mx-auto px-3 lg:px-5 flex flex-col gap-3 relative z-10">
        ...
        <div className="animate-fade-in-up flex flex-col xl:flex-row xl:items-end xl:justify-between gap-3">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-[var(--color-accent)]/20 bg-[var(--color-accent)]/10 text-[10px] uppercase tracking-[0.22em] text-[var(--color-accent)]">
              Prediction Lab
            </div>
          ...
          <div className="flex items-center gap-3 ml-0 flex-wrap xl:justify-end">
            {result && (
              <div className="flex items-center gap-2 px-3 py-2 rounded-2xl border border-[var(--border)] bg-[var(--color-surface)] text-[12px] text-[var(--color-text-secondary)]">
                {result.cached ? (
                  <>
                ...
                <Database size={11} className="text-[var(--color-text-secondary)]" />
                <span>{result.universe_size} stocks scanned</span>
              </div>
            )}
            <div className="inline-flex items-center gap-1 rounded-full border border-[var(--border)] bg-[var(--color-surface)] p-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
              <button
                type="button"
                ...
                className={`inline-flex items-center gap-1.5 rounded-full px-3.5 py-2 text-[10px] font-semibold uppercase tracking-[0.18em] transition-all ${
                  predictionMode === "previous_close"
                    ? "bg-[var(--color-surface)]/60 text-[var(--color-text-primary)] shadow-sm"
                    : "text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]"
                }`}
              >
                <Clock size={11} className={predictionMode === "previous_close" ? "text-[var(--color-accent)]" : "text-[var(--color-text-secondary)]"} />
                Previous Close
              </button>
              <button
                type="button"
              ...
              >
                <ArrowUpRight size={11} className={predictionMode === "live" ? "text-[var(--color-success)]" : "text-[var(--color-text-secondary)]"} />
                Live
              </button>
            </div>
            <button
          ...
          <div className="flex flex-col gap-4 animate-slide-in-left">
            ...
            <Card variant="compact">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div>
                  <label className="text-[10px] text-[var(--color-text-secondary)] uppercase tracking-wider font-semibold block mb-1.5">
                    Show Top N
                  </label>
                  <select
                    id="screener-top-n"
                      ...
                      <button
                        ...
                        className={`py-1.5 rounded-lg text-xs font-semibold border transition-all ${
                          directionFilter === d
                            ...
                            : "border-[var(--border)] text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] hover:bg-[var(--color-surface)]"
                        }`}
                      >
              ...
              <Card variant="compact">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-[var(--color-text-secondary)] flex items-center gap-1.5"><TrendingUp size={11} className="text-[var(--color-success)]" /> Bullish picks</span>
                    <span className="text-[var(--color-success)] font-semibold">{longs}</span>
                  </div>
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-[var(--color-text-secondary)] flex items-center gap-1.5"><TrendingDown size={11} className="text-[var(--color-danger)]" /> Bearish picks</span>
                    <span className="text-[var(--color-danger)] font-semibold">{shorts}</span>
                  </div>
                  <div className="h-px bg-[var(--border)] my-1" />
                  <div className="flex justify-between text-xs">
                    <span className="text-[var(--color-text-secondary)]">Universe scanned</span>
                    <span className="text-[var(--color-text-muted)] font-mono">{result.universe_size}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-[var(--color-text-secondary)]">Model date</span>
                    <span className="text-[var(--color-text-muted)] font-mono">{result.date}</span>
                  </div>
                  {result.model_metrics && (
                    <>
                      <div className="h-px bg-[var(--border)] my-1" />
                      <div className="text-[10px] uppercase tracking-wider text-[var(--color-text-secondary)] font-semibold">Validation</div>
                      <div className="flex justify-between text-xs">
                        <span className="text-[var(--color-text-secondary)]">Hit rate</span>
                        <span className="text-[var(--color-text-muted)] font-mono">{((result.model_metrics.hit_rate ?? 0) * 100).toFixed(0)}%</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="text-[var(--color-text-secondary)]">Spearman IC</span>
                        <span className="text-[var(--color-text-muted)] font-mono">{(result.model_metrics.spearman_ic ?? 0).toFixed(3)}</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="text-[var(--color-text-secondary)]">RMSE</span>
                        <span className="text-[var(--color-text-muted)] font-mono">{(result.model_metrics.rmse ?? 0).toFixed(4)}</span>
                      </div>
                    </>
                  )}
                </div>
            ...
            <Card variant="compact">
              ...
              <div className="flex flex-col gap-3 text-xs text-[var(--color-text-secondary)] leading-relaxed">
                ...
                <Link
                  href="/backtest"
                  className="flex items-center gap-1.5 text-[var(--color-cta)] hover:text-[var(--color-accent)] transition-colors font-semibold"
                >
          ...
          <div className="animate-fade-in-up min-w-0">
              ...
              <div className="grid md:grid-cols-3 gap-4 mb-6">
                {topSignals.map((s, i) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-[10px] uppercase tracking-[0.18em] text-[var(--color-text-secondary)]">
            ...
            <Card variant="compact">
              ...
              {(!loading || hasLoaded) && signals.length > 0 && (
                <div className="overflow-x-auto rounded-2xl border border-[var(--border)] bg-[var(--color-surface)]/20">
                  <div className="min-w-[1120px]">
                    <div className="grid grid-cols-[18rem_8rem_8rem_11rem_9rem_1fr] gap-x-4 px-5 py-4 text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--color-text-secondary)] border-b border-[var(--border)] bg-[var(--color-surface)]/30">
                      {TABLE_HEADERS.map((h) => (
                        <div
                          key={h}
                      ...
                      {signals.map((s, i) => (
                        <div
                          ...
                          className={`grid grid-cols-[18rem_8rem_8rem_11rem_9rem_1fr] gap-x-4 px-5 py-4.5 items-center rounded-2xl border border-[var(--border)]/50 animate-fade-in-up ${
                            i % 2 === 0 ? "bg-[var(--color-surface)]" : "bg-[var(--color-surface)]/95"
                          } hover:bg-[var(--color-surface)]/90 transition-colors`}
                          style={{ animationDelay: `${i * 40}ms` }}
                        >
                          <div className="min-w-0">
                            <div className="flex flex-col gap-1 min-w-0">
                              <span className="text-[11px] font-mono text-[var(--color-text-secondary)]">#{String(i + 1).padStart(2, "0")}</span>
                              <div className="flex items-baseline gap-2 min-w-0">
                                <span className="font-mono font-bold text-[var(--color-text-inverse)] text-[15px] tracking-[0.06em]">{s.symbol}</span>
                                <span className="text-[var(--color-text-muted)] text-[13px] truncate">{s.name || s.symbol}</span>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="pr-2">
                            <span className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-[12px] font-bold border ${
                              s.side === "long"
                                ? "text-[var(--color-success)] bg-[var(--color-success)]/10 border-[var(--color-success)]/25"
              ...
              {(!loading || hasLoaded) && signals.length === 0 && !error && (
                <div className="flex flex-col items-center justify-center py-16 gap-4">
                  ...
                  <div className="text-center">
                    <p className="text-[var(--color-text-secondary)] text-sm font-semibold">No predictions yet</p>
            ...
            <div className="mt-4 flex items-start gap-2 px-1 text-[10px] text-[var(--color-text-secondary)] leading-relaxed">
              ...
              <p>
                These predictions are generated by a machine learning model trained on historical price data and are for <strong className="text-[var(--color-text-secondary)]">informational purposes only</strong>. Past model performance does not guarantee future results. Always conduct your own research before making any investment decisions.
              </p>
            </div>
          </div>
        </div>
